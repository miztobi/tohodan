<script lang="ts">
  ...
  // 最後尾ユーザーのデータをベースにした進捗表示用（'tail'タグを持つユーザーを優先）
  const tailParticipant = $derived(
    participants.find(p => p.role === "tail") || (participants.length > 0 ? participants.reduce((prev, curr) => (prev.progress < curr.progress ? prev : curr)) : null)
  );

  const segmentData = $derived.by(() => {
    if (!tailParticipant) return null;
    
    let accumulatedDistance = 0;
    const currentDistance = (tailParticipant.progress / 100) * totalDistance;
  ...
  async function handleUpdateLocation() {
    ...
    try {
      ...
      if (user) {
        // 最後尾タグ(tail)を付けて保存
        await saveProgress(user.uid, coords, progress, "tail");
      }
  ...
  $effect(() => {
    if (tailParticipant) {
      appState.progress = tailParticipant.progress;
      appState.lastUpdated = tailParticipant.timestamp;
    }
  });
</script>
  ...
  <div class="w-full max-w-md space-y-6">
    <header class="pt-6 pb-2 flex items-center justify-between">
      <div class="flex items-center gap-4">
        <div class="p-3 bg-purple-600 rounded-2xl shadow-xl">
          <ShieldCheck class="text-white" size={32} />
        </div>
        <div>
          <h1 class="text-3xl font-black text-white tracking-tighter">tohodan <span class="text-purple-500">TAIL</span></h1>
          <p class="text-sm font-bold text-slate-400 uppercase tracking-widest">Tail Device</p>
        </div>
      </div>
    ...
    {#if segmentData?.segment}
      <div class="bg-gradient-to-br from-purple-600 to-fuchsia-700 text-white p-6 rounded-[2rem] shadow-2xl flex items-center justify-between border-b-4 border-purple-800">
        <div class="space-y-2">
    ...
    <div class="space-y-4 pt-4">
      <Button 
        ...
        class="w-full py-6 text-2xl font-black shadow-2xl rounded-[2rem] flex items-center justify-center gap-3 active:scale-95 transition-transform bg-purple-500 hover:bg-purple-600 border-none text-white"
      >
